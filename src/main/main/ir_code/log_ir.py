...
class DroneDataLogger(Node):
    ...
    def gps_callback(self, msg):
        """ Store camera frame timestamp """
        self.last_lat = msg.latitude_deg
        self.last_lon = msg.longitude_deg
...
def calculate_position_and_angle(cx, cy):
    ...
    with open(CSV_FILENAME, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([thetaX, thetaY, r, "Hawktuah"])

    return r, thetaX, thetaY

def generate_line(cx, cy, drone_data):
    ...
    position = np.array([drone_data.last_lon, drone_data.last_lat, drone_data.last_z])

    # Convert angles to direction vector
    direction = np.array([
        np.cos(drone_data.last_yaw) * np.cos(thetaY + drone_data.last_roll),
        np.sin(drone_data.last_yaw) * np.cos(thetaY + drone_data.last_roll),
        np.sin(thetaX + drone_data.last_pitch)
    ])
...
def process_frame(frame):
    ...
    if contours:
        ...
        if M["m00"] != 0:
            ...
            #lines.append(generate_line(cx,cy,drone_data))
