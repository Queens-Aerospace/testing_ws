...
class OffboardControl(Node):
    ...
    def __init__(self) -> None:
        ...
        self.bucket_subscriber = self.create_subscription(
            String, '/bucket/movement_instructions', self.bucket_callback, 10)
    ...
    def bucket_callback(self, msg):
        """Callback function for bucket location"""
        self.bucket_state = msg.data
    ...
    def bucket_landing(self):
        # End early if nothing is detected
        #if self.no_bucket_counter > 50:
            #self.state = 2
            #return
        
        bucket_items = self.bucket_state.split()

        # Move based on bucket location
        if "NoBucket" in bucket_items:
            self.publish_position_setpoint(self.vehicle_local_position.x, self.vehicle_local_position.y, self.vehicle_local_position.z)
            self.no_bucket_counter += 1
            return
